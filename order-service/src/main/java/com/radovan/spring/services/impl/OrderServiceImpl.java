...
public class OrderServiceImpl implements OrderService {
	...
	private final String CUSTOMER_SERVICE_URL = "http://localhost:8082/";

	private final String CART_SERVICE_URL = "http://localhost:8083/";

	private final String PRODUCT_SERVICE_URL = "http://localhost:8084/";
	...
	public OrderDto addOrder() {
		...
		ResponseEntity<JsonNode> customerResponse = deserializeConverter
				.getJsonNodeResponse(CUSTOMER_SERVICE_URL + "customers/currentCustomer");
		...
		ResponseEntity<JsonNode> cartResponse = deserializeConverter
				.getJsonNodeResponse(CART_SERVICE_URL + "cart/validateCart/" + cartId);
		...
		ResponseEntity<JsonNode> shippingAddressResponse = deserializeConverter
				.getJsonNodeResponse(CUSTOMER_SERVICE_URL + "addresses/addressDetails/" + shippingAddressId);
		...
		List<JsonNode> cartItems = deserializeConverter
				.getJsonNodeList(CART_SERVICE_URL + "items/allItemsByCartId/" + cartId);
		for (JsonNode cartItem : cartItems) {
			...
			ResponseEntity<JsonNode> productResponse = deserializeConverter
					.getJsonNodeResponse(PRODUCT_SERVICE_URL + "products/productDetails/" + productId);
			...
			if (quantity > unitStock) {
				...
				String productUrl = PRODUCT_SERVICE_URL + "products/orderUpdateProduct/" + productId;
		...
		String clearCartUrl = CART_SERVICE_URL + "cart/clearCart";
